...
###################################################################################
# 트윗을 입력받아, 감정 분석 결과를 출력하는 코드입니다
# 현재 지원 행정구역: 서초구

# INPUT: 작업 디렉토리 / twitter_cr.py 로부터의 csv 파일 (result.csv)
# OUTPUT: 없음

# 추가해야 할 기능
# -성능 개선(리스트->딕셔너리 등으로 교체)
# -감성 사전 키워드 보완(현재는 다소 지엽적임)
# -키워드 추출 알고리즘 보완(a,b,c,d 행)
# -원본트윗 오탈자 normalize 방식 개선
# -다른 행정구역 지원
# -DB와의 연계
###################################################################################
...
for n in df['store name']:
    store_name.append(n)
for s in df['tweet']:
    norm_list.append(preprocessing(s))
